...
public class Parser {
        ...
        if (see(LCURLY)) {
            ...
            if (!see(SEMI)) {
                if (seeBasicType() || seeReferenceType()) {
                    forInit.add(new JForInitDeclaration(line, variableDeclarators(type())));
                } else {
            ...
            //3.9
            JStatement tryBlock = blockStatement();
            ArrayList<JStatement> catches = new ArrayList<JStatement>();
            ArrayList<JFormalParameter> exceptions = new ArrayList<JFormalParameter>();

            //several catches put together
            ...
            while (have(CATCH)) {
                ...
                if (see(LCURLY)) {
                    ...
                    //if not see SEMI, means not empty
                    exceptions.add(exception);
                    catches.add(blockStatement());
                }
            }
            //[finally{   }]
            if (have(FINALLY)) {
                JStatement finallyStatement = blockStatement();
                return new JTryCatchFinallyStatement(line, tryBlock, catches, 
                                    exceptions, finallyStatement);
            } else {
                //condtion if no [finally] appears
                return new JTryCatchFinallyStatement(line, tryBlock, catches, exceptions);
            }
        } else if (have(SWITCH)) {
            JExpression parExpression = parExpression();
        	mustBe(LCURLY);
        	ArrayList<JSwitchBlockStatement> switchBlockStatement = 
        		new ArrayList<JSwitchBlockStatement>();
        	while (!see(RCURLY) && !see(EOF)) {
        		switchBlockStatement.add(switchBlockStatementGroup());
        	}
        	mustBe(RCURLY);
        	return new JSwitchStatement(line, parExpression, switchBlockStatement);
        } else if (have(DEFAULT)) {
             have(COLON);
             return new JDefaultStatement(line);
        } else if (have(RETURN)) {
            if (have(SEMI)) {
            ...
            //throw (NEW) --> creator();
            JExpression throwExpression = primary(); 
            mustBe(SEMI);
            ...
            //3.7
            if (have(SEMI)) {
                return new JBreakStatement(line, null);
            } else {
                String identifier = scanner.previousToken().image();
                mustBe(SEMI);
                return new JBreakStatement(line, identifier);
            }
        } else if (have(CONTINUE)) {
            //3.8
            if (have(SEMI)) {
                return new JContinueStatement(line, null);
            } else {
                String identifier = scanner.previousToken().image();
                mustBe(SEMI);
                return new JContinueStatement(line, identifier);
            }
        } else if (have(SEMI)) {
            return new JEmptyStatement(line);
    ...
    //3.4 private method for JSwitchBlockStatementGroup
    private JSwitchBlockStatement switchBlockStatementGroup() {
    	int line = scanner.token().line();
    	ArrayList<JExpression> switchLabel = new ArrayList<JExpression>();
    	ArrayList<JStatement> blockStatement = new ArrayList<JStatement>();
    	ArrayList<JStatement> defaultstatement = new ArrayList<JStatement>();

    	switchLabel.add(switchLabel());
    	while (!see(CASE) && see(DEFAULT) && !see(RCURLY)) {
    		defaultstatement.add(blockStatement());
    	}
    	while (see(CASE)) {
    		switchLabel.add(switchLabel());
    	}
    	...
    	return new JSwitchBlockStatement(line, switchLabel, 
    		blockStatement);
    }
    ...
    private JExpression conditionalExpression() {
        ...
        while (more) {
            if (have(QUES)) {
                ...
                if (have(COLON)) {
                    JExpression result = expression();
                    return new JConditionalExpression(line, lhs, condition, result);
                } else {
                    return new JConditionalExpression(line, lhs, condition, null);
                }
            } else {
                more = false;
            }
