...
enum TokenKind {
    ...
    CATCH("catch"), CONTINUE("continue"), DEFAULT("default"), DO("do"),
    DOUBLE("double"), FINAL("final"), FINALLY("finally"), FOR("for"),
    IMPLEMENTS("implements"), INTERFACE("interface"), LONG("long"),
    SWITCH("switch"), THROW("throw"), THROWS("throws"), TRY("try"),

    // Operators
    ASSIGN("="), DEC("--"), EQUAL("=="), GT(">"), INC("++"),
    LAND("&&"), LE("<="), LNOT("!"), MINUS("-"), PLUS("+"),
    PLUS_ASSIGN("+="), STAR("*"), DIV("/"), REM("%"), SHL("<<"),
    SHR(">>"), USHR(">>>"), OR("|"), XOR("^"), AND("&"), NOT("~"),
    //question 2.3
    QUES("?"), NOT_ASSIGN("!="), DIV_ASSIGN("/="), MIN_ASSIGN("-="),
    STAR_ASSIGN("*="), REM_ASSIGN("%="), SHR_ASSIGN(">>="),
    USHR_ASSIGN(">>>="), GE(">="), SHL_ASSIGN("<<="), LT("<"), XOR_ASSIGN("^="),
